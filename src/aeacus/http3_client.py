...
class MyQuicConnectionProtocol(QuicConnectionProtocol):
    ...
    def transmit(self) -> None:
        ...
        for data, addr in self._quic.datagrams_to_send(now=self._loop.time()):
            log(f'Sending {len(data)} bytes to {addr}')
...
class HttpClient(MyQuicConnectionProtocol):
    ...
    def http_event_received(self, event: H3Event) -> None:
        log(f'Received http event: {type(event)}')
        if isinstance(event, HeadersReceived):
            log(f'HTTP headers received: {event.headers[:3]} ...')
